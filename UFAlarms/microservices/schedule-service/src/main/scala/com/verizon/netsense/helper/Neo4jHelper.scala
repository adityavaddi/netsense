...
class Neo4jHelper extends Neo4jProductionDb with Instrumented {
  ...
  private[this] val LSSLNeo4jReadTimer: Timer = metrics.timer("lss-neo4j-read-flow-timer")
  ...
  def getDataFromGraphDb(cypher: String, innerParams: util.HashMap[String, Object]) = {
    LSSLNeo4jReadTimer.time {
      val propsMap = new util.HashMap[String, Object]()
      propsMap.putIfAbsent(Constants.PROPS, innerParams)
