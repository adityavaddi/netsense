...
class TagConnector(override val connector: KeySpaceDef) extends Database[TagConnector](connector) {
  outer =>
  ...
  object WhatIfTagTable extends ConcreteWhatIfTag with connector.Connector

}


object TagConnector extends TagConnector(Defaults.connection)
