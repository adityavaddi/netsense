...
object AkkaStreamUtil extends Logging {
  ...
  lazy val akkaGraphDecider: Supervision.Decider = {
    case NonFatal(ex) => log.error("### akkaGraphDecider: Exception found in the Stream " + ex.printStackTrace()); Supervision.Resume
    case ex: Exception => log.error("### akkaGraphDecider: Unhandled Exception seen " + ex.printStackTrace()); Supervision.stop;
  }
  ...
  lazy val queryFlowDecider: Supervision.Decider = {
    ...
    case ex: Exception => log.error("### queryFlowDecider: Unhandled Exception seen " + ex.printStackTrace()); Supervision.Resume
  }
