...
import com.verizon.netsense.whatifservice.model.casel.{AppRequest, MessageIdAndResponseTopic, RequestBody}
...
object ObjectMapperUtil {
  ...
  def toJson(value: Any): String = {println("=========",mapper.writeValueAsString( value ))
  ...
  def generateAppRequestForValidationError(msgid: String, responsetopic: String, errormsg: String): AppRequest =
    AppRequest(
      msgid,
      responsetopic,
      RequestBody(null, null, null, `type` = "validationfailed", model = null, null, None, null, null, None,null)
    )
