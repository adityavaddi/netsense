...
-define (PROCS,[zmqpushclient]).


start_link() -> lager:info("starting dcc queue_observer"), gen_server:start_link({local, ?MODULE}, ?MODULE, [], []).
...
check_local_queue(Proc) ->
   ...
   lager:info("This ~p(~p) has ~p messages in queue~n",[Proc, Pid, Quelen]),
   TopOffenders = recon:proc_window(reductions, 5, 100),
   lager:info("Top 5 offenders are ~p~n",[TopOffenders]).
