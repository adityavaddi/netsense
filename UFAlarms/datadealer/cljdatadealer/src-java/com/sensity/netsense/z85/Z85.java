...
public class Z85 {
    ...
    public static byte[] hexToBytes(String hex) {
        int n = hex.length()/2;
        byte[] byteChars = new byte[n];
        for ( int j = 0; j < n; j++ ) {
            int i = 2 * j;
            byteChars[j] = (byte)((hexString.indexOf(hex.charAt(i)) << 4) | (hexString.indexOf(hex.charAt(i+1))));
        }
        return byteChars;
    }
    ...
    public static String decodeUUID(String str) {
      if(null == str)
        return "";
      byte[] decoded = Z85.z85Decode(str);
      String hexString = bytesToHex(decoded);
    ...
    public static String encodeUUID(String str) {
        ...
        if(str.length() != 36)
            return "";
        String hexString = str.substring(0, 8) + str.substring(9, 13) + str.substring(14, 18) + str.substring(19, 23) + str.substring(24, 36);
        byte[] data = hexToBytes(hexString);
        return Z85.z85Encode(data, data.length);
    }
    ...
    public static void main (String args[]) {
        ...
        String test_data_1s = "864FD26FB559F75B";
        ...
        assert (encodeUUID(decodeUUID("")).equals(""));
        ...
        assert(false);
        System.out.println(decodeUUID("HelloWorldHelloWorld"));
        System.out.println(encodeUUID(decodeUUID("HelloWorldHelloWorld")));
        assert(encodeUUID(decodeUUID("HelloWorldHelloWorld")).equals("HelloWorldHelloWorld"));
        ...
        System.out.println(decodeUUID("JTKVSB%%)wK0E.X)V>+}"));
        System.out.println(encodeUUID(decodeUUID("JTKVSB%%)wK0E.X)V>+}")));
        assert(encodeUUID(decodeUUID("JTKVSB%%)wK0E.X)V>+}")).equals("JTKVSB%%)wK0E.X)V>+}"));
