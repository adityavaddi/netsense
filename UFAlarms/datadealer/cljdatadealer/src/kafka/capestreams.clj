...
(defn cape-streams-config
  [config]
  (let [brokers (string/join "," (map #(str % ":" (:port config)) (:host config)))
        props {StreamsConfig/APPLICATION_ID_CONFIG (str "cape-worker-" (:suffix config (java.util.UUID/randomUUID)))
...
(defn start
  ...
  (info "Starting Kafka cape stream...")
  (let [ks (KafkaStreams. builder config)
        connected (try
                    ...
                    true
                    (catch Exception e
                      (spyf :error "Kafka cape stream connection failed: %s" e)
                      false))]
    (when (not connected)
      (errorf "Could not connect to Kafka cape steam. Sleeping %d, then retrying." 250)
      (Thread/sleep 250)
