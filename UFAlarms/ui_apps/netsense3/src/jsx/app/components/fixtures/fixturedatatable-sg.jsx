...
var Fixturedatatable = React.createClass({
  ...
  columns: [
    {name:"Icon", field:"fixtureType", id:"fixtureType", 
  ...
  componentDidUpdate() {
    this.grid.setData(this.props.fixtures, "fixtureid");
  ...
  componentDidMount() {
    var that = this;
    var searchList = [];
    $("#fixture-table").height(helpers.calcHeight(100,-150));
    
    // initialize grid and dataview
    this.dataView = new Slick.Data.DataView();
    this.grid = new Slick.Grid("#fixture-table", this.dataView, this.columns, this.gridOptions);
    this.grid.setData(this.props.fixtures, "fixtureid");
    this.grid.render();
    ...
    this.grid.onSelectedRowsChanged.subscribe(function(e, args) {
      ...
      if (selectedData.length == 1) {
        ReactBootstrap.Dispatcher.emit(that.props.component+"list.select", selectedData[0].fixtureid);
      } else {
        ReactBootstrap.Dispatcher.emit(that.props.component+"list.multiSelect", 
          selectedData.map(function(fixture) {
    ...
    this.grid.onSort.subscribe(function(e, args){ // args: sort information. 
      ...
      that.props.fixtures.sort(function(a, b){
          var result = 
              a[field] > b[field] ? 1 :
              a[field] < b[field] ? -1 :
              0;
    ...
    $("#fixture-search").keyup(function (e) {
      // clear on Esc
      if (e.which == 27) {
        this.value = "";
      }
...
// initialize the model after all the events have been hooked up
  ...
  this.dataView.setItems(this.props.fixtures, "fixtureid");
    ...
    return (
      <div>
        <div style={{textAlign:"right",fontSize:"16px",display:"none"}}>
          <input type="text" name="fixture-search" id="fixture-search" 
              style={{width:"40%",border:"1px solid silver",marginBottom:"10px"}} placeholder=" Search..." />
          </div>
        <div id="fixture-table" width="100%">
        </div>
      </div>
    );
  }
