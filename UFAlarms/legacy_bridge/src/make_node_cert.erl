...
-module(make_node_cert).
-export([all/0, make_node_cert/1]).

%%from Scala
%%val cn = s"$newCertName/name=Sensity/emailAddress=support@sensity.com"
make_node_cert(NodeId) ->
    LongTime = calendar:gregorian_days_to_date(calendar:date_to_gregorian_days(date())+15*365),
    Validity = {date(), LongTime},

    CaCertFile  = filename:join(code:priv_dir(legacy_bridge), "ssl/old/sensity.crt"),
    ServerKeyFile  = filename:join(code:priv_dir(legacy_bridge), "ssl/old/server-key.pem"),
    {ok, SensityCaCert} = file:read_file(CaCertFile),
    {ok, ServerKey} = file:read_file(ServerKeyFile),
    ...
    ClientCa = erl_make_certs:make_cert([{key,ServerKey},{issuer, SensityCaCert}, {validity, Validity}, 
                     {subject, [{name, NodeId ++ "/name=","Sensity","/emailAddress=support@sensity.com"}|Subject]}]),
    ServerCa = erl_make_certs:make_cert([{issuer, SensityCaCert}, {validity, Validity}, 
                     {subject, [{name, "Server/name=","Sensity","/emailAddress=support@sensity.com"}|Subject]}]),
    ...
    ok = file:write_file(filename:join(CPath, "cacerts.pem"), SensityCaCert),
    erl_make_certs:write_pem(SPath, "cert", ServerCa),
    ok = file:write_file(filename:join(SPath, "cacerts.pem"), SensityCaCert),
